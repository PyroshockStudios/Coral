...
public class MemberMethodTest
{
    ...
    // should throw exception when calling
    public unsafe DummyStruct* DummyStructPtrTestWrong(DummyStruct* InValue)
    {
        return InValue;
    }

    public IntPtr DummyStructPtrTestGood(IntPtr InValue)
    {
        unsafe
        {
            DummyStruct* ptr = (DummyStruct*)InValue.ToPointer();
            ptr->X *= 2;
            ptr->Y *= 2.0f;
            ptr->Z *= 2;
            return (IntPtr)ptr;

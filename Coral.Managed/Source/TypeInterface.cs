...
internal static class TypeInterface
{
    ...
    [UnmanagedCallersOnly]
    internal static int GetTypeGenericArgument(int InType, int InGenericIndex)
    {
        if (!s_CachedTypes.TryGetValue(InType, out var type) || type == null)
            return -1;
        return s_CachedTypes.Add(type.GetGenericArguments()[InGenericIndex]);
    }
    [UnmanagedCallersOnly]
    internal static int GetTypeGenericTypeDefinition(int InType)
    {
        if (!s_CachedTypes.TryGetValue(InType, out var type) || type == null)
            return -1;
        return s_CachedTypes.Add(type.IsGenericType ? type.GetGenericTypeDefinition() : type);
    }
